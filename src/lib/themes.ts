export type Theme = {
  ...
  name: string
  /**
   * Which way the browser should draw the parts of the page it owns — the number
   * spinners, the select popup, the caret, the scrollbars. Four of these themes are
   * light, and a UA told otherwise draws dark-mode chrome on a white surface.
   */
  scheme: 'light' | 'dark'
  ...
  text: string
  /**
   * The quiet ink: slot labels, legends, hints, timestamps. Its alpha is set per theme
   * rather than shared, because dark ink at half opacity on a pale ground loses far
   * more contrast than light ink at half opacity on a dark one. Each value is the one
   * that carries an 11px label past 4.5:1 over the worst part of that theme's own
   * gradient where a label actually lands.
   */
  muted: string
...
export const themes: Theme[] = [
  ...
  },
  {
    ...
    name: 'Sunset',
    scheme: 'light',
    // The magenta this theme ended on was dark enough that even full-strength ink
    // could not clear 4.5:1 over it — the ground was the limit, not the ink. Lifted
    // just far enough that the darkest part of it carries a label.
    background:
      'radial-gradient(100% 80% at 20% 15%, #ffd08a 0%, transparent 58%),' +
      'radial-gradient(110% 90% at 85% 20%, #ffa588 0%, transparent 60%),' +
      'radial-gradient(120% 100% at 60% 100%, #f591b8 0%, transparent 65%),' +
      'linear-gradient(155deg, #ffc79b 0%, #ffa2bf 50%, #d29ad2 100%)',
    text: '#3a1024',
    muted: 'rgba(58, 16, 36, 0.78)',
...
export function buildCustomTheme(custom: CustomTheme): Theme {
  ...
  return {
    id: CUSTOM_THEME_ID,
    name: 'Custom',
    // Light ink means a dark ground, which is what the browser needs to hear.
    scheme: ink.scheme,
    background,
    text: ink.text,
    muted: ink.muted,
